...
model_path = '~/deepdreamer/' # substitute your path here
net_fn   = model_path + 'deploy.prototxt'
param_fn = model_path + 'snapshot_iter_6225.caffemodel.caffemodel'
...
net = caffe.Classifier('tmp.prototxt', param_fn,
                       mean = np.float32([85.1, 89.23, 107.78]), # ImageNet mean, training set dependent
...
guide = np.float32(PIL.Image.open('~/deepdreamer/test_images/faces/trump.jpg'))
img = np.float32(PIL.Image.open('~/deepdreamer/test_images/faces/obama.jpg'))
#showarray(guide)
...
output=deepdream(net, img, end=end, objective=objective_guide)
